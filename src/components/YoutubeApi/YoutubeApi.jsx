...
import YoutubeVideosList from '../YoutubeVideosList/YoutubeVideosList.jsx'
 ...
 class YoutubeApi extends Component {
	...
	checkDuplicate() {
		var litVideos = this.state.getVids();
		var litVideosDB = this.state.listVids;
		
		for (var i  = 0; i < gvid.length; i++) {
			for (var j = 0; j < gvidDB.length; j++) {
	        	if (getVids[i] != getVidsFromDB[j]) {
	          		 list = [];
	          		 list.push(litVideos[j]);
		    	} else {
		    		this.state.getVidsFromDB();
		   	 	} 
   	 		}	
	 	}
	}

	getVids(channelId,nextPageToken) {

		if(nextPageToken != null) {
			$.get(
			this.state.apiUrl+"playlistItems",
				{
					part:"snippet",
					key:this.state.key,
			...
			var list = this.jsonToObject(data.items);
			if(list != null ){
			 	this.sendToDb(list);
			}
			this.getVids(channelId,nextPageToken);
			});
		}
	}
	...
	jsonToObject(data) {
		...
		if (data.length > 0 ) {
			...
			for (var i = 0; i < data.length; i++) {
				var isFromYoutube = data[i].hasOwnProperty('snippet');
				var title  = isFromYoutube ? data[i].snippet.title : data[i].title ;
